...
export function Separation() {
  ...
  const handleStartTask = async (saleId: string, allowUnpaid = false) => {
    try {
      setErrorMsg("");
      setOpeningTaskId(saleId);
      const res = await apiFetch(`/api/separation/sales/${saleId}/start`, {
        method: "POST",
      ...
      if (res.status === 409 && e.code === "UNPAID_CONFIRM_REQUIRED" && isPrivileged) {
        const proceed = window.confirm("Esta venda ainda não está paga. Deseja continuar a separação mesmo assim?");
        if (proceed) await handleStartTask(saleId, true);
        return;
      }
  ...
  const fetchTaskDetails = async (taskId: string) => {
    try {
      ...
      if (res.ok) setActiveTask(await res.json());
        ...
        const e = await res.json().catch(() => ({}));
        setErrorMsg(e.error || "Erro ao abrir a separação.");
      }
    } catch(err) {
      console.error(err);
  ...
  const openQueueSale = async (sale: any) => {
    if (openingTaskId) return;
    const unpaid = String(sale?.paymentStatus || "PENDING").toUpperCase() !== "PAID";
    if (unpaid) {
      if (!isPrivileged) {
        setErrorMsg("Esta venda ainda não está paga. Somente Admin ou Master pode liberar a separação.");
        return;
      }
      const proceed = window.confirm("Esta venda ainda não está paga. Deseja continuar a separação mesmo assim?");
      if (!proceed) return;
    }
    if (sale.taskId) await fetchTaskDetails(sale.taskId);
    else await handleStartTask(sale.id, unpaid);
  };
  ...
  const confirmItem = async (itemId: string, quantityExpected: number) => {
    try {
      ...
      if (res.ok) fetchTaskDetails(activeTask.id);
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="text-2xl font-semibold text-white flex items-center gap-2">
            <PackageSearch className="text-brand-gold" /> Separando Pedido
          </h2>
              ...
              <CardContent className="flex w-full flex-col items-center gap-4 p-4 sm:flex-row">
                ...
                <div className="flex-1 w-full text-center sm:text-left">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2 justify-center sm:justify-start">
                    <Badge variant="outline" className="border-primary/30 text-primary uppercase">Prateleira: {i.shelfName || "N/A"}</Badge>
                    {i.reservedStock !== undefined && <Badge variant="info" className="uppercase">Reservado: {i.reservedStock}</Badge>}
                    <Badge variant="secondary" className="uppercase">Qtd Vendida: {i.quantityExpected}</Badge>
                  </div>
                </div>
        ...
        <div className="mt-8 flex justify-end">
          <Button className="h-auto rounded-xl px-8 py-4 text-lg font-bold shadow-lg shadow-primary/20" disabled={hasPending} onClick={completeTask}>
            Finalizar Separação
          </Button>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-2xl font-semibold text-white flex items-center gap-2">
          <PackageSearch className="text-brand-gold" /> Fila de Separação
        </h2>
              ...
              <CardContent className="flex flex-col gap-5 p-5">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    {s.paymentStatus !== 'PAID' && <Badge variant="warning" className="uppercase">Não pago</Badge>}
                  </div>
                </div>
                <Button
                  variant="outline"
                  ...
                  onClick={(e) => { e.stopPropagation(); openQueueSale(s); }}
                >
