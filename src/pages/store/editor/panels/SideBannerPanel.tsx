...
export function SideBannerPanel() {
  ...
  useEffect(() => {
    ...
    const next = [0, 1].map((index) => ({ ...FALLBACKS[index], ...(saved[index] || {}) }));
    setItems(next);
  ...
  const handleFile = async (index: number, file?: File) => {
    if (!file) return;
    setUploading(index);
    try { update(index, { url: await compressImage(file, BANNER_COMPRESS_OPTS) }); }
    catch (error: any) { toast.error(error.message || "Erro ao processar a imagem."); }
    ...
    <PanelShell title="Editar os 2 banners" onClose={ctx.closePanel} onSave={save} saving={saving} wide>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {items.map((item, index) => {
          const objectPosition = `${item.posX ?? 50}% ${item.posY ?? 50}%`;
          return (
            <div key={index} className="rounded-xl border border-stone-200 p-2.5">
              ...
              <div className="mb-2 aspect-[19/7] overflow-hidden rounded-lg border border-rose-100 bg-[#fff7f8]">
                {item.url ? (
                  <img src={item.url} alt={`Prévia banner ${index + 1}`} style={{ objectPosition, "--banner-object-position": objectPosition } as any} className="h-full w-full object-cover" />
                ) : (
              ...
              <label className="mb-2 flex cursor-pointer items-center justify-center gap-1.5 rounded-lg border border-dashed border-stone-300 py-2 text-xs font-semibold text-stone-500 hover:border-rose-400 hover:text-rose-500">
                ...
                <input type="file" accept="image/*" className="hidden" disabled={uploading !== null} onChange={(event) => { handleFile(index, event.target.files?.[0]); event.currentTarget.value = ""; }} />
              </label>
