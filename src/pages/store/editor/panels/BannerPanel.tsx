...
// Banners do carrossel do topo da home. Cada item é {url, link, title, posX} —
      ...
      <div className="space-y-3">
        {banners.map((b, i) => (
          <div key={b.__key} className="rounded-lg border border-stone-200 p-2.5">
            ...
            <input value={b.title || ""} onChange={(e) => update(b.__key, { title: e.target.value })} placeholder="Título (opcional — some sem CTA se vazio)" className="w-full rounded-md border border-stone-300 p-2 text-sm outline-none focus:border-amber-500" />
          </div>
        ))}
